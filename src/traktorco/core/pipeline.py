...
- **Empty Stem Detection (section 10.1):** immediately after extracting
  ...
  unconditionally -- in both ``--verify fast`` (the default) and
  ``--verify smart`` -- since it protects *any* mode from analyzing
  silence.
...
from traktorco.audio.loader import extract_drum_stem, is_drum_stem_empty, load_window
...
from traktorco.nml.parser import BatchTrackRef, NmlParser
from traktorco.nml.stems import has_stem_flag, resolve_stem_path
...
def _resolve_analysis_source(
    ...
    1. If ``entry.flags`` does not carry the native-stem bit (spec
       section 9.1), fall back to ``fallback_path`` unchanged.
    2. Otherwise, predict the sidecar path (``nml.stems.resolve_stem_path``),
    ...
    3. Otherwise, extract the isolated Drums/Rhythm stream from the
    ...
    4. v2.2 Empty Stem Detection (spec section 10.1): run a lightning-fast
...
def run_pipeline(
    ...
    analysis_path, temp_stem_wav = _resolve_analysis_source(
        entry, track_path, nml_path, stems_dir=config.stems_dir
    )
    try:
