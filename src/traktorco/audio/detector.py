...
# CSV column names for v1.8 data export (matches the dict keys written by
# _write_csv_row). v1.9 added track_peak_db and track_perceived_db.
_CSV_FIELDNAMES = [
    "track_title",
    "beat",
    "time_ms",
...
def _write_csv_row(
    ...
    """Write a single candidate's telemetry row to the CSV writer."""
...
def detect_events(
    ...
    # v1.8 data export: write per-candidate telemetry to CSV
...
def _export_csv(
    ...
    """Append per-candidate telemetry rows to a CSV file.

    Writes a header row on first open; subsequent calls append data rows.
    """
    ...
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        ...
        if not file_exists:
            writer.writeheader()
        for sc in scored:
            status = status_by_beat.get(sc.candidate.beat_index, "REJECTED_THRESHOLD")
            _write_csv_row(
                writer,
                track_title,
                sc.candidate,
    ...
    logger.info("Exported %d candidate row(s) to %s", len(scored), csv_path)
