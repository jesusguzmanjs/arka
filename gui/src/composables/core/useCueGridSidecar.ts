...
import { useConfigState } from "./useConfigState.ts";
import { useRunState, type RunSummary } from "./useRunState.ts";
import { useErrorReporter } from "./useErrorReporter.ts";
import { useAnalysisSession } from "../collection/useAnalysisSession.ts";
import { preparePlayerForAnalysis } from "../player/useTrackMetadata.ts";
import { isUnanalyzedTrack } from "../../utils/trackAnalysis.ts";
import { showAppToast } from "./useAppToast.ts";
import type { CueGridConfig } from "../../types/config.ts";
import type { LibraryTrack } from "../../types/library.ts";
import type { SidecarMessage } from "../../types/sidecar.ts";
import type {
  SmartPlaylistCompileResult,
  SmartPlaylistPayload,
} from "../../types/smartPlaylist.ts";
...
export function useCueGridSidecar() {
  ...
  async function runSelectedTracks(
    tracks: readonly Pick<LibraryTrack, "location_path" | "title" | "bpm">[],
  ): Promise<void> {
    const analyzedTracks = tracks.filter((track) => !isUnanalyzedTrack(track));
    const skippedCount = tracks.length - analyzedTracks.length;
    if (skippedCount > 0) {
      showAppToast(
        `${skippedCount} track(s) skipped. Lacks BPM/Grid data in the collection. Please analyze or run Check Consistency in Traktor.`,
        "warning",
      );
    }
    if (analyzedTracks.length === 0) return;

    const trackPaths = analyzedTracks.map((track) => track.location_path);
    await runAnalysis("track", trackPaths[0], `${analyzedTracks.length} selected tracks`, trackPaths);
  }
  ...
  async function runAnalysis(
    ...
    const unlistenClose = await listen<number | null>("analysis-close", (event) => {
      ...
      if (succeeded) {
        ...
        useErrorReporter().triggerError(
          backendError ?? `${successLabel} Auto Cue failed. Review the analysis log and try again.`,
        );
      }
    ...
    try {
      ...
      useErrorReporter().triggerError(err);
      cleanupListeners();
