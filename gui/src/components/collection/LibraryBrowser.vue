...
import { ALL_TRACKS_CONTEXT, useLibraryState } from "../../composables/collection/useLibraryState.ts";
import { useConfigState } from "../../composables/core/useConfigState.ts";
import { usePlayerState } from "../../composables/player/useTrackMetadata.ts";
import { useColumnResize } from "../../composables/core/useColumnResize.ts";
...
const hasAnalyzableSelection = computed(() => (
  selectedMetadataTracks.value.some((track) => !isUnanalyzedTrack(track))
));
...
const isContextAutoCueEnabled = computed(() => {
...
function bpmLabel(track: LibraryTrack): string {
  return isUnanalyzedTrack(track) ? "" : track.bpm!.toFixed(1);
}
...
function sendToRemixStudio(): void {
  ...
  if (isUnanalyzedTrack(target.track)) {
    showAppToast(MISSING_BPM_GRID_MESSAGE, "warning");
    return;
  }
    ...
    <div class="flex shrink-0 items-center justify-between border-b border-zinc-800 px-4 py-2.5">
      ...
      <div class="flex items-center gap-3">
        ...
        <button
            ...
            :disabled="selectedMetadataTracks.length === 0 || !hasAnalyzableSelection || props.disabled"
          ...
          >
            ...
            >
              <div
                  ...
                  :aria-label="isUnanalyzedTrack(sortedTracks[virtualRow.index])
                    ? `Unanalyzed track: ${sortedTracks[virtualRow.index].artist} – ${sortedTracks[virtualRow.index].title}`
                    : `Load ${sortedTracks[virtualRow.index].artist} – ${sortedTracks[virtualRow.index].title}`"
                  :title="isUnanalyzedTrack(sortedTracks[virtualRow.index])
                    ? MISSING_BPM_GRID_MESSAGE
                    : sortedTracks[virtualRow.index].is_flex_grid ? flexGridTooltip : undefined"
              ...
              >
                ...
                <span
                  class="pointer-events-none px-2 text-left font-mono text-xs tabular-nums"
                  :class="isUnanalyzedTrack(sortedTracks[virtualRow.index]) ? 'text-warning/80' : 'text-muted'"
                  role="cell"
                  :aria-label="isUnanalyzedTrack(sortedTracks[virtualRow.index]) ? 'Unanalyzed: no BPM or Grid data' : undefined"
                >
                  <span v-if="isUnanalyzedTrack(sortedTracks[virtualRow.index])" aria-hidden="true">⚠ </span>{{ bpmLabel(sortedTracks[virtualRow.index]) }}
                </span>
