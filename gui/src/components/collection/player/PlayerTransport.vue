...
import type { PlayerCue } from "../../../composables/player/usePeaksMarkers.ts";
...
<template>
  <div class="flex items-center gap-1 px-1" role="group" aria-label="Player transport and hotcue jump pads">
    <button type="button" :disabled="!hasTrack" class="text-xs font-mono font-semibold px-2 py-1 rounded border bg-zinc-800 text-zinc-100 border-zinc-700 hover:bg-zinc-700 hover:text-accent disabled:opacity-40 disabled:hover:text-zinc-100 disabled:hover:bg-zinc-800 disabled:cursor-not-allowed transition-colors" :title="isPlaying ? 'Pause' : 'Play'" :aria-label="isPlaying ? 'Pause' : 'Play'" @click="emit('play')">
      {{ isPlaying ? "❚❚" : "▶" }}
    </button>
    <button type="button" :disabled="!isPlaying" class="text-xs font-mono font-semibold px-2 py-1 rounded border bg-zinc-800 text-zinc-100 border-zinc-700 hover:bg-zinc-700 hover:text-accent disabled:opacity-40 disabled:hover:text-zinc-100 disabled:hover:bg-zinc-800 disabled:cursor-not-allowed transition-colors" title="Stop" aria-label="Stop" @click="emit('stop')">■</button>
    <template v-for="n in 8" :key="`pad-${n}`">
      ...
      <button v-else type="button" :disabled="!hasTrack" class="flex items-center justify-center text-xs font-mono font-semibold w-8 h-8 rounded border bg-zinc-900/50 text-zinc-700 border-zinc-800/50 hover:bg-zinc-800 hover:text-accent disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Add Cue" @click="emit('addCue', n - 1)">{{ n }}</button>
    </template>
  </div>
</template>
