...
import { fetchTrackMetadata, usePlayerState } from "../../composables/player/useTrackMetadata.ts";
...
import { activeAudioEngine } from "../../composables/player/useGlobalAudio.ts";
import { STEM_LANES, useStemPeaks } from "../../composables/remix-studio/useStemPeaks.ts";
...
const editFadeInMs = shallowRef(0);
const editFadeOutMs = shallowRef(0);
...
function clampEditFades(range: NonNullable<typeof activeLoopRange.value>): void {
  ...
  const fadeInMs = Math.max(0, Number(editFadeInMs.value) || 0);
  const fadeOutMs = Math.max(0, Number(editFadeOutMs.value) || 0);
  const totalFadeMs = fadeInMs + fadeOutMs;

  if (totalFadeMs <= loopDurationMs) {
    editFadeInMs.value = fadeInMs;
    editFadeOutMs.value = fadeOutMs;
    return;
  }

  const scale = totalFadeMs === 0 ? 0 : loopDurationMs / totalFadeMs;
  editFadeInMs.value = fadeInMs * scale;
  editFadeOutMs.value = fadeOutMs * scale;
}
...
function updatePadFades(): void {
  clearFadeEvents();
  if (!loopFadeGain || !isPadEditMode.value || !activeLoopRange.value) {
...
watch(editingPad, (target) => {
  ...
  editFadeInMs.value = target.pad.settings.fadeInMs ?? 0;
  editFadeOutMs.value = target.pad.settings.fadeOutMs ?? 0;
  setPadEditSelection();
}, { immediate: true });
...
function observeWaveformResize(container: HTMLDivElement | null): void {
  ...
  waveformResizeObserver = new ResizeObserver(() => {
    ...
    // --- NUEVO: Forzamos el repintado de las líneas para que cojan la nueva altura ---
    if (isMultiTrackMode.value) {
      paintStemBeatGrids();
    } else if (activePeaks.value) {
      paintBeatGrid(activePeaks.value);
    } else {
      syncGridOpacity();
    }
  });
...
function handleZoomWheel(event: WheelEvent): void {
  ...
  // MAGIA: Permitimos alejar hasta un 10% extra de la duración total para que nunca se quede corto
  newSeconds = Math.max(MIN_VISIBLE_SECONDS, Math.min(totalSeconds + 0.02, newSeconds));
  ...
  // Si la vista ya engloba la pista entera, la anclamos a la izquierda para que no flote
  let newStartTime = 0;
  if (actualSeconds < totalSeconds) {
    newStartTime = Math.max(0, Math.min(timeAtCursor - ratio * actualSeconds, totalSeconds - actualSeconds));
  }
...
function startFadeDrag(event: MouseEvent, mode: "fade-in" | "fade-out"): void {
  if (event.button !== 0 || !isPadEditMode.value || !activeLoopRange.value) return;
...
function handleSelectionDrag(event: MouseEvent): void {
  ...
  if (drag.mode === "draw") {
    ...
    if (isPadEditMode.value && !drag.didCreateLoop) {
      editFadeInMs.value = 0;
      editFadeOutMs.value = 0;
    }
  ...
  await new Promise<void>((resolve, reject) => {
    Peaks.init(peaksOptions, (error: Error | null, instance: unknown) => {
      ...
      if (view && duration.value > 0) {
        view.setZoom({ seconds: duration.value + 0.02 });
        view.setStartTime(0);
      }
...
<style scoped>
.stem-editor { --stem-controls-width: 70px; display: flex; min-width: 650px; min-height: 420px; padding: 1.5rem; flex: 1 1 auto; flex-direction: column; overflow-x: hidden; overflow-y: auto; background: #232326; }
