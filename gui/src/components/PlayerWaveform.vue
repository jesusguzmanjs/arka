...
<template>
  <div class="bg-zinc-950 border border-zinc-800 rounded h-10 w-full overflow-hidden">
    <div ref="overviewElement" class="h-full w-full" />
  </div>
  <div class="flex items-center gap-2 w-full h-40">
    <div class="relative flex-1 bg-zinc-950 border border-secondary/40 rounded h-full overflow-hidden" @wheel.prevent="emit('wheel', $event)">
      ...
      <div v-if="isLoading || isSaving" class="absolute inset-0 flex items-center justify-center bg-zinc-950/80 z-10"><span class="text-xs font-mono animate-pulse text-zinc-400">{{ isSaving ? "Saving changes to NML…" : "Loading track…" }}</span></div>
      <div v-else-if="!hasTrack" class="absolute inset-0 flex items-center justify-center bg-zinc-950"><span class="text-xs font-mono text-zinc-600">Double-click a track in the library to preview</span></div>
    </div>
