...
class TestWriteCues:
    ...
    def test_preserves_autogrid_cue_byte_for_byte(self, working_nml):
        ...
        NmlWriter(NmlParser(working_nml)).write_cues(
            KNOWN_TRACK_PATH, [_cue("Intro End", 6997.217, hotcue=0)]
        )
    ...
    def test_never_removes_or_reorders_existing_children(self, working_nml):
        ...
        before_entry = before_tree.getroot().find("./COLLECTION/ENTRY")
        before_children = [(child.tag, child.get("NAME")) for child in before_entry]

        NmlWriter(NmlParser(working_nml)).write_cues(
            KNOWN_TRACK_PATH, [_cue("Drop", 27568.836, hotcue=0)]
        )

        after_tree = ET.parse(working_nml)
        after_entry = after_tree.getroot().find("./COLLECTION/ENTRY")
        after_children = [(child.tag, child.get("NAME")) for child in after_entry]

        # Every original child, in its original order, is still present
        # as a prefix of the new children list -- only appended, nothing
    ...
    def test_formats_floats_with_six_decimal_places(self, working_nml):
        ...
        assert 'START="27568.836000"' in raw_xml
    ...
    def test_does_not_overwrite_an_existing_backup_on_second_run(self, working_nml):
        ...
        NmlWriter(NmlParser(working_nml)).write_cues(
            KNOWN_TRACK_PATH, [_cue("Outro", 240142.234, hotcue=1)]
        )
        assert backup_path.read_bytes() == backup_after_first_run

    def test_raises_on_hotcue_slot_conflict(self, working_nml):
        # The AutoGrid cue occupies HOTCUE=-1 (unbound) in this fixture,
        # so seed an explicit conflict against a slot we assign ourselves.
        writer = NmlWriter(NmlParser(working_nml))
        writer.write_cues(KNOWN_TRACK_PATH, [_cue("Intro End", 6997.217, hotcue=0)])
