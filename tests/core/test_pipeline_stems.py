...
from traktorco.core.pipeline import _resolve_analysis_source
...
class TestResolveAnalysisSource:
    ...
    def test_uses_extracted_drum_stem_when_sidecar_exists(self, tmp_path):
        ...
        with patch("traktorco.core.pipeline.resolve_stem_path") as mock_resolve:
            ...
            with patch(
                "traktorco.core.pipeline.extract_drum_stem", return_value=fake_wav
            ) as mock_extract:
                path, temp = _resolve_analysis_source(
    ...
    def test_falls_back_when_extraction_fails(self, tmp_path):
        ...
        with patch("traktorco.core.pipeline.resolve_stem_path") as mock_resolve:
            ...
            with patch(
                "traktorco.core.pipeline.extract_drum_stem",
    ...
    def test_forwards_stems_dir_override_to_resolve_stem_path(self, tmp_path):
        ...
        with patch(
            "traktorco.core.pipeline.resolve_stem_path", return_value=None
        ) as mock_resolve:
    ...
    def test_falls_back_when_extracted_stem_is_empty(self, tmp_path):
        ...
        with patch("traktorco.core.pipeline.resolve_stem_path") as mock_resolve:
            ...
            with patch(
                ...
                with patch(
                    "traktorco.core.pipeline.is_drum_stem_empty", return_value=True
                ) as mock_is_empty:
                    path, temp = _resolve_analysis_source(
                        entry, "/music/test.flac", nml_path
                    )
    ...
    def test_uses_extracted_stem_when_not_empty(self, tmp_path):
        ...
        with patch("traktorco.core.pipeline.resolve_stem_path") as mock_resolve:
            ...
            with patch(
                ...
                with patch(
                    "traktorco.core.pipeline.is_drum_stem_empty", return_value=False
                ):
                    path, temp = _resolve_analysis_source(
                        entry, "/music/test.flac", nml_path
                    )
