...
from cuegrid.engine import matches_rule, matches_rules
...
@pytest.mark.parametrize(
    "rule",
    [
        {"field": "bpm", "operator": "equals", "value": 120},
        {"field": "bpm", "operator": "greater_than", "value": 120},
        {"field": "bpm", "operator": "less_than", "value": 120},
        {"field": "bpm", "operator": "between", "value": {"min": 100, "max": 140}},
    ],
)
@pytest.mark.parametrize("bpm", [0, 0.0])
def test_unanalyzed_bpm_never_matches_bpm_rules(rule, bpm):
    assert not matches_rule({"bpm": bpm}, rule)
    assert not matches_rules({"bpm": bpm}, [rule])
