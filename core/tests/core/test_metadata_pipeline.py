...
from cuegrid.audio.metadata import MetadataWriteError
...
class TestMetadataUpdatePipeline:
    def test_commits_nml_before_best_effort_physical_writes(self, tmp_path):
        ...
        with patch(
            "cuegrid.core.pipeline.write_metadata_to_file",
    ...
    def test_does_not_attempt_physical_writes_when_atomic_nml_write_fails(self, tmp_path):
        ...
        with patch(
            "cuegrid.core.pipeline.NmlWriter.write_metadata_batch",
            side_effect=OSError("NML locked"),
        ), patch("cuegrid.core.pipeline.write_metadata_to_file") as write_file:
