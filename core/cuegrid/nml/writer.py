...
class NmlWriter:
    ...
    def update_track_hotcues(
        ...
        ``grid_anchor_ms`` is supplied, it is written only when the target has
        exactly one Grid marker; unanalyzed and Flex Grid entries retain their
        existing Grid state. When ``bpm`` is supplied, it is written only when
        the target contains a direct ``TEMPO`` child. Non-zero BPM values must
        be finite and within the inclusive range ``[50, 200]``.
    ...
    def _validate_bpm(bpm: float | None) -> float | None:
        """Validate an optional manual BPM update before mutating XML.

        ``0.0`` is the parser's sentinel for a track without tempo analysis,
        so it represents no BPM update rather than an invalid user edit.
        """
        if bpm is None:
            return None
        if bpm == 0:
