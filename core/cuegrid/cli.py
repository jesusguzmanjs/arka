...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.batch_save is not None:
        ...
        try:
            ...
            if run_batch_save_pipeline is None:
                from cuegrid.engine.pipeline import run_batch_save_pipeline as batch_save_run
    ...
    if args.update_metadata is not None:
        ...
        if run_metadata_update_pipeline is None:
            from cuegrid.engine.pipeline import (
    ...
    if len(args.track_paths) == 1:
        ...
        if run_pipeline is None:
            from cuegrid.engine.pipeline import run_pipeline as pipeline_run
            run_pipeline = pipeline_run
        if serialize_gui_payload is None:
            from cuegrid.engine.pipeline import serialize_gui_payload as pipeline_serialize
        ...
        if run_batch_pipeline is None:
            from cuegrid.engine.pipeline import run_batch_pipeline as pipeline_run_batch
